...
def api_passage(request):
    ...
    try:
        ...
        # Try by slug
        try:
            book = BibleBook.objects.get(
                slug__iexact=book_name.lower().replace(" ", "-")
            )
        except BibleBook.DoesNotExist:
    ...
    data = {
        "reference": ref,
        "normalized": f"{book.name} {chapter}"
        + (f":{start_verse}" if start_verse else "")
        + (f"-{end_verse}" if end_verse and end_verse != start_verse else ""),
        "book": {
            "name": book.name,
            "slug": book.slug,
        },
...
def parse_reference(ref):
    ...
    if not match:
        return {
            "error": 'Invalid reference format. Use: "Book Chapter" or "Book Chapter:Verse" or "Book Chapter:Verse-Verse"'
        }
